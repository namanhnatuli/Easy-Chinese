...
const serverEnvSchema = publicEnvSchema.extend({
  ...
  GEMINI_MODEL_WEIGHTS: z.string().optional(),
  GEMINI_MAX_RETRIES: z.coerce.number().int().min(1).max(100).optional(),
  GEMINI_TIMEOUT_MS: z.coerce.number().int().min(1_000).max(300_000).optional(),
  GEMINI_DEFAULT_TEMPERATURE: z.coerce.number().min(0).max(1).optional(),
  GEMINI_DEFAULT_MAX_OUTPUT_TOKENS: z.coerce.number().int().min(1).max(65_536).optional(),
...
export function getServerEnv() {
  cachedServerEnv ??= serverEnvSchema.parse({
    ...
    GEMINI_API_KEYS: normalizeOptionalEnv(process.env.GEMINI_API_KEYS),
    GEMINI_MODEL_WEIGHTS: normalizeOptionalEnv(process.env.GEMINI_MODEL_WEIGHTS),
    GEMINI_MAX_RETRIES: normalizeOptionalEnv(process.env.GEMINI_MAX_RETRIES),
    GEMINI_TIMEOUT_MS: normalizeOptionalEnv(process.env.GEMINI_TIMEOUT_MS),
    GEMINI_DEFAULT_TEMPERATURE: normalizeOptionalEnv(process.env.GEMINI_DEFAULT_TEMPERATURE),
    GEMINI_DEFAULT_MAX_OUTPUT_TOKENS: normalizeOptionalEnv(process.env.GEMINI_DEFAULT_MAX_OUTPUT_TOKENS),
