-- ==========================================
-- FIX: FK violation in apply_vocab_sync_row exception handler
-- ==========================================
  ...
  begin
    ...
    matched_radicals as (
      select
        ...
        min(radicals.id::text)::uuid as radical_id
      ...
      left join public.radicals on
        lower(trim(radicals.radical)) = desired_aliases.alias
        or lower(trim(coalesce(radicals.display_label, ''))) = desired_aliases.alias
        or lower(trim(coalesce(radicals.han_viet_name, ''))) = desired_aliases.alias
        or lower(trim(coalesce(radicals.meaning_vi, ''))) = desired_aliases.alias
        or exists (
          select 1
          from unnest(coalesce(radicals.variant_forms, array[]::text[])) as variants(variant_value)
